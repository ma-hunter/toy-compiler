...
void toObjectCode(shared_ptr<CodeNode> head) {
    ...
    do {
        auto[l, l_type] = parseOperand(TheContext, val_table, cur->op1);
        auto[r, r_type] = parseOperand(TheContext, val_table, cur->op2);
        switch (cur->kind) {
            case ASSIGNOP: {
                ...
                if (l->getType()->isPointerTy()) {
                    l = builder_stack.back().CreateLoad(
            ...
            case CALL: {
                ...
                val_table[get<string>(cur->res.data)] = {
                        builder_stack.back().CreateCall(fn_type, fn, parameters, get<string>(cur->res.data)),
